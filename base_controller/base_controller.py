...
#        self.launch = roslaunch.parent.ROSLaunchParent(uuid, [os.environ['LOCOSIM_DIR'] + "/ros_impedance_controller/launch/ros_impedance_controller_stdalone.launch"],roslaunch_args=[roslaunch_args])
        ...
        self.robot = getRobotModel(self.robot_name_, generate_urdf = True)
        

        threading.Thread.__init__(self)
								
        # instantiating objects
        self.ros_pub = RosPub(self.robot_name_,True)                    
        self.joint_names = ""
        self.u = Utils()	
        ...
        self.q_des[:12] = conf.robot_params[self.robot_name_]['q_0']   
        ...
        #self.sub_contact = ros.Subscriber("/"+self.robot_name_+"/contacts_state", ContactsState, callback=self._receive_contact, queue_size=1, buff_size=2**24,  tcp_nodelay=True)       
        #self.sub_pose = ros.Subscriber("/"+self.robot_name_+"/base_state", BaseState, callback=self._receive_pose, queue_size=1,buff_size=2**24, tcp_nodelay=True) no longer used
        self.sub_pose = ros.Subscriber("/"+self.robot_name_+"/ground_truth", Odometry, callback=self._receive_pose, queue_size=1, tcp_nodelay=True)     
        self.sub_jstate = ros.Subscriber("/"+self.robot_name_+"/joint_states", JointState, callback=self._receive_jstate, queue_size=1,  tcp_nodelay=True)                  
...
#        self.grForcesW[11] =  msg.states[3].wrenches[0].force.z
    ...
    def deregister_node(self):
        ...
        os.system(" rosnode kill /"+self.robot_name_+"/ros_impedance_controller")    
    ...
    def freezeBase(self, flag):
        ...
        model_state.model_name = self.robot_name_
    ...
    def startupProcedure(self):
            ...
            # set joint pdi gains
            self.pid.setPDs(conf.robot_params[self.robot_name_]['kp'], conf.robot_params[self.robot_name_]['kd'], 0.0) 
           
  
            
            if (self.robot_name_ == 'hyq'):                        
    ...
    def initVars(self):
        ...
        self.basePoseW_log = np.empty((6, conf.robot_params[self.robot_name_]['buffer_size']))*nan
        self.baseTwistW_log = np.empty((6,conf.robot_params[self.robot_name_]['buffer_size'] ))*nan
        self.q_des_log = np.empty((self.robot.na, conf.robot_params[self.robot_name_]['buffer_size'] ))*nan    
        self.q_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] )) *nan   
        self.qd_des_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] ))*nan    
        self.qd_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] )) *nan                                  
        self.tau_ffwd_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] ))*nan    
        self.tau_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] ))*nan                                  
        self.grForcesW_log = np.empty((self.robot.na,conf.robot_params[self.robot_name_]['buffer_size'] ))  *nan 
        self.time_log = np.empty((conf.robot_params[self.robot_name_]['buffer_size']))*nan
        self.constr_viol_log = np.empty((4,conf.robot_params[self.robot_name_]['buffer_size'] ))*nan
        self.time = 0.0
        self.log_counter = 0

    def logData(self):
        if (self.log_counter<conf.robot_params[self.robot_name_]['buffer_size'] ):
...
def talker(p):
    ...
    #loop frequency       
    rate = ros.Rate(1/conf.robot_params[p.robot_name_]['dt']) 
...
#    omega = 5000*RPM2RAD
    ...
    while p.time <3.0:  
        ...
        # controller                             
        p.tau_ffwd = conf.robot_params[p.robot_name_]['kp'] * np.subtract(p.q_des,   p.q)  - conf.robot_params[p.robot_name_]['kd']*p.qd + p.gravity_comp  
...
#            break;
        ...
        p.time = p.time + conf.robot_params[p.robot_name_]['dt']			
    ...
    p.pid.setPDs(conf.robot_params[p.robot_name_]['kp'], conf.robot_params[p.robot_name_]['kd'], 0.0) 
    ros.sleep(1.0)                
    print ("Shutting Down")                 
