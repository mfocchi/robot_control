...
class BaseController(threading.Thread):
    ...
    def __init__(self):  
       ...
       #clean up previous process                
        ...
        self.launch = roslaunch.parent.ROSLaunchParent(uuid, [os.environ['LOCOSIM_DIR'] + "/ros_impedance_controller/launch/ros_impedance_controller_solo12.launch"])
...
#        self.launch = roslaunch.parent.ROSLaunchParent(uuid, [os.environ['LOCOSIM_DIR'] + "/ros_impedance_controller/launch/ros_impedance_controller_stdalone.launch"],roslaunch_args=[roslaunch_args])
        ...
        self.ros_pub = RosPub("solo12",True) #start only graphic stuff 
        self.joint_names = ""
        self.u = Utils()
...
#        self.grForcesW[11] =  msg.states[3].wrenches[0].force.z
    ...
    def deregister_node(self):
        ...
        os.system(" rosnode kill /solo12/ros_impedance_controller")    
...
def talker(p):
    ...
    p.initVars()        
    ros.sleep(0.1) #need this delay to get he joint names
    p.pid = PidManager(p.joint_names) #I start after cause it needs joint names filled in by receive jstate callback
    p.pid.setPDs(10.0, 0.1, 0.0)
    ...
    while True:  
        ...
        # controller                             
        #p.tau_ffwd = 10 * np.subtract(p.q_des,   p.q)  - 0.1*p.qd 
