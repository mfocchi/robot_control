          ...
          *varargout_1)
{
  ...
  for (b_i = 0; b_i <= i; b_i++) {
    ...
    y = states->data[6 * b_i + 2];
    scale = params->b * params->b;
    b_scale = t * t;
    y = (scale + b_scale) - y * y;
    scale = muDoubleScalarSqrt(1.0 - y * y / (4.0 * scale * b_scale));
    Fr_l->data[b_i] = t * muDoubleScalarSin(absxk) * scale;
    py->data[b_i] = y / (2.0 * params->b);
    pz->data[b_i] = -t * muDoubleScalarCos(absxk) * scale;
  ...
  if (params->obstacle_avoidance) {
    ...
    for (b_i = 0; b_i < i; b_i++) {
      y = p->data[3 * b_i + 2] - -4.5;
      scale = p->data[3 * b_i + 1] - 3.0;
      ...
      pz->data[varargout_1->size[1]] = -p->data[3 * b_i] + muDoubleScalarSqrt
        ((2.25 - 3.0 * (y * y)) - scale * scale);
