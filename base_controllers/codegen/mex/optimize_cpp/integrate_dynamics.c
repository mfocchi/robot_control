                                 ...
                                 const real_T u3[3], real_T varargout_1[6])
{
  ...
  if (t1 > 0.0) {
    ...
    r = _mm_loadu_pd(&Ftot[0]);
    r1 = _mm_loadu_pd(&p[0]);
    _mm_storeu_pd(&Ftot[0], _mm_add_pd(r, r1));
    Ftot[2] += p[2];
  }
  a = 1.0 / params_m;
  x_tmp = x[1] * n_pz_l1;
  ...
  a_tmp = x[5] * x[5];
  b_px_tmp = x[3] * x[3];
  py = (((((4.0 * muDoubleScalarPower(x[1], 4.0) * scale -
            8.0 * b_scale * x[2] * x[4] * x[5]) +
           4.0 * b_a_tmp * a_tmp_tmp * a_tmp) -
          6.0 * b_a_tmp * scale * py2b) -
         2.0 * b_a_tmp * a_tmp * py2b) +
        8.0 * x[1] * x[2] * x[4] * x[5] * py2b) +
       3.0 * scale * x_tmp_tmp;
  b_scale = px_tmp_tmp * b_scale * px_l1_sinpsi;
  p_tmp = c_a_tmp * c_a_tmp;
  y = 16.0 * muDoubleScalarPower(params_b, 4.0) *
      muDoubleScalarPower(x[1], 5.0) * muDoubleScalarPower(px_l1_sinpsi, 3.0);
  b_y = x[4] * py2b;
  b_t = b_x_tmp_tmp * px_l1_sinpsi;
  absxk = x[3] * py2b;
  t1 = b_x_tmp * b_a_tmp * px_l1_sinpsi;
  p[0] = ((((2.0 * x[4] * n_pz_l1 * x[3] - x[1] * b_px_tmp * px_l1_tmp) -
            px_tmp * py / b_scale) -
           x_tmp_tmp * px_tmp * p_tmp / y) +
          absxk * pz_tmp * c_a_tmp / t1) +
         b_y * px_tmp * c_a_tmp / b_t;
  p[1] = (scale - a_tmp) / params_b;
  p[2] = ((((x_tmp * b_px_tmp + 2.0 * x[4] * x[3] * px_l1_tmp) +
            pz_tmp * py / b_scale) +
           x_tmp_tmp * pz_tmp * p_tmp / y) -
          b_y * pz_tmp * c_a_tmp / b_t) +
         absxk * px_tmp * c_a_tmp / t1;
  r = _mm_loadu_pd(&Ftot[0]);
  r1 = _mm_loadu_pd(&p[0]);
  _mm_storeu_pd(&Ftot[0], _mm_sub_pd(_mm_mul_pd(_mm_set1_pd(a), r), r1));
  Ftot[2] = a * Ftot[2] - p[2];
  varargout_1[0] = x[3];
  varargout_1[1] = x[4];
  varargout_1[2] = x[5];
  ...
  r = _mm_mul_pd(r, _mm_set1_pd(b_t));
  r1 = _mm_loadu_pd(&b_x[3]);
  r1 = _mm_mul_pd(r1, _mm_set1_pd(p_tmp));
  r = _mm_add_pd(r, r1);
  r1 = _mm_loadu_pd(&b_x[6]);
  r1 = _mm_mul_pd(r1, _mm_set1_pd(t1));
  r = _mm_add_pd(r, r1);
  _mm_storeu_pd(&varargout_1[3], r);
  varargout_1[5] = (b_x[2] * b_t + b_x[5] * p_tmp) + b_x[8] * t1;
}
