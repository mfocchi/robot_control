...
class QuadrupedJumpController(QuadrupedController):
    ...
    def init_pid_tuning_ui(self):
        ...
        def create_triple_slider(label, row, initial_values, max_value, resolution):
        ...
        self.kp_sliders = create_triple_slider("KP", 0, self.initial_kp, 60, 0.1)
        self.kd_sliders = create_triple_slider("KD", 1, self.initial_kd, 2, 0.01)
        self.ki_sliders = create_triple_slider("KI", 2, self.initial_ki, 5, 0.1)

        # Create sliders for kp_lin, kd_lin, kp_ang, kd_ang
        self.kp_lin_sliders = create_triple_slider("KP LIN", 3, self.initial_kp_lin, 1000, 0.1)
    ...
    def update_pid_values(self):
        ...
        def get_slider_values(sliders, max_values):
            return np.clip([slider.get() for slider in sliders], 0, max_values)

        # Get values from each set of three sliders as arrays
        kp_values = get_slider_values(self.kp_sliders, 60.)
        kd_values = get_slider_values(self.kd_sliders, 2.)
        ki_values = get_slider_values(self.ki_sliders, 5.)
        ...
        kp_lin_array = get_slider_values(self.kp_lin_sliders, 1000.)
    ...
    def initVars(self):
        ...
        if self.real_robot:
            ...
            self.initial_kp = conf.robot_params[self.robot_name]['kp_real']
            self.initial_kd = conf.robot_params[self.robot_name]['kd_real']
            self.initial_ki = conf.robot_params[self.robot_name]['ki_real']
            self.initial_kp_lin = conf.robot_params[self.robot_name]['kp_lin_real']
            self.initial_kd_lin = conf.robot_params[self.robot_name]['kd_lin_real']
            self.initial_kp_ang = conf.robot_params[self.robot_name]['kp_ang_real']
            self.initial_kd_ang = conf.robot_params[self.robot_name]['kd_ang_real']

        else:
            # Load initial PID values from configuration
            self.initial_kp = conf.robot_params[self.robot_name]['kp']
            self.initial_kd = conf.robot_params[self.robot_name]['kd']
            self.initial_ki = conf.robot_params[self.robot_name]['ki']
            self.initial_kp_lin = conf.robot_params[self.robot_name]['kp_lin']
            self.initial_kd_lin = conf.robot_params[self.robot_name]['kd_lin']
            self.initial_kp_ang = conf.robot_params[self.robot_name]['kp_ang']
            self.initial_kd_ang = conf.robot_params[self.robot_name]['kd_ang']
