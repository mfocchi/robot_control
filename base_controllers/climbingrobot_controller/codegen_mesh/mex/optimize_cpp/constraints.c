                 ...
                 struct2_T *number_of_constr, struct3_T *solution_constr)
{
  ...
  number_of_constr->initial_final_constraints = 1.0;
  ...
  absxk = muDoubleScalarAbs(
      solution_constr->p->data[3 * (solution_constr->p->size[1] - 1)] - pf[0]);
  if (absxk > 3.3121686421112381E-170) {
    b_y = 1.0;
    b_scale = absxk;
  } else {
  ...
  absxk = muDoubleScalarAbs(
      solution_constr->p->data[3 * (solution_constr->p->size[1] - 1) + 1] -
      pf[1]);
  if (absxk > b_scale) {
    t = b_scale / absxk;
    b_y = b_y * t * t + 1.0;
  ...
  absxk = muDoubleScalarAbs(
      solution_constr->p->data[3 * (solution_constr->p->size[1] - 1) + 2] -
      pf[2]);
  if (absxk > b_scale) {
    t = b_scale / absxk;
    b_y = b_y * t * t + 1.0;
  ...
  solution_constr->final_error_discrete = b_scale * muDoubleScalarSqrt(b_y);
  ...
  if (number_of_constr->via_point > 0.0) {
    ...
    ineq_data = ineq->data;
    ineq_data[i] =
        -solution_constr->p->data[3 * ((int32_T)(params->N_dyn / 2.0) - 1)] +
