                        ...
                        const struct0_T *in3)
{
  ...
  if (in3->Fr_l_fine->size[1] == 1) {
    loop_ub = in2->size[1];
  } else {
    loop_ub = in3->Fr_l_fine->size[1];
  }
  i = in1->size[0] * in1->size[1];
  in1->size[1] = loop_ub;
  emxEnsureCapacity_real_T(in1, i);
  ...
  stride_1_1 = (in3->Fr_l_fine->size[1] != 1);
  for (i = 0; i < loop_ub; i++) {
    in1_data[i] = in3->Fr_l_fine->data[i * stride_1_1] *
                  in2_data[6 * (i * stride_0_1) + 4];
  }
}
