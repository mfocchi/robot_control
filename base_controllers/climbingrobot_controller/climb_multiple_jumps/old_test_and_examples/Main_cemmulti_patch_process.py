...
# Shared optimizer reference inherited by worker processes via fork (no pickling needed)
...
def _eval_with_timing_process(idx_inp):
    """Top-level function for ProcessPoolExecutor workers.
    Uses _global_optimizer set by _worker_init initializer.
    """
    idx, inp = idx_inp
    t0 = time.time()
    result = _global_optimizer.eval_pop(inp)
    elapsed = time.time() - t0
    return idx, result, elapsed

def _worker_init(optimizer, initialized_inner_opt_params):
    """Runs once per spawned worker process: sets the global optimizer
    and patches the module-level inner_opt_params with the fully
    initialized values from the main process (mesh_x, patch_side_z, etc.)
    that are only populated after initialize_terrain_data() runs.
    Each worker creates its own MATLAB engine lazily on first eval_pop call,
    and keeps it open for the lifetime of the worker (across iterations).
    """
    import atexit
    global _global_optimizer
    _global_optimizer = optimizer
    # Update the module-level inner_opt_params so create_inner_opt_params_copy()
    # sees the fully initialized dict (mesh_x, patch_side_z, patch_side_y, etc.)
    import params as _params_mod
    _params_mod.inner_opt_params.update(initialized_inner_opt_params)
    # Ensure the MATLAB engine is cleanly closed when this worker process exits
    # (triggered by ProcessPoolExecutor.shutdown or any other clean exit)
    atexit.register(close_matlab_engines)
...
def main():
    ...
    if setting["COMPUTATION_MODE"]:
        ...
        # Expose optimizer globally so worker processes inherit it via fork (no pickling)
        global _global_optimizer
        _global_optimizer = optimizer
        ...
        # Build a serializable snapshot of inner_opt_params for workers.
        # Exclude matlab.double objects (not picklable) and None mesh values
        # that are already re-created inside create_inner_opt_params_copy().
        _serializable_keys = [
            'm', 'num_params', 'int_method', 'N_dyn', 'FRICTION_CONE',
            'int_steps', 'b', 'g', 'w1', 'w2', 'w3', 'T_th',
            'obstacle_avoidance', 'jump_clearance', 'debug',
            'mesh_x', 'mesh_y', 'mesh_z',
            'cost_x', 'cost_y', 'cost_z',
            'patch_side_z', 'patch_side_y',
        ]
        _worker_inner_opt = {k: inner_opt_params[k] for k in _serializable_keys if k in inner_opt_params}
        
        # Create the process pool ONCE before the main loop.
        # Using 'spawn': each worker is a fresh Python process (no MATLAB fork-safety issues).
        # The initializer sets _global_optimizer in each worker once at startup.
        # Workers persist across all CEM iterations, so each worker's MATLAB engine
        # is created lazily on the first eval_pop call and reused for all subsequent calls.
        ...
        _process_executor = ProcessPoolExecutor(
            ...
            initargs=(_global_optimizer, _worker_inner_opt)
        ) if flag_thread else None
        ...
        for k in range(cem_params.cem_iters):
            ...
            # ============================
            # flag_thread == TRUE: multi-process evaluation (ProcessPoolExecutor + spawn)
            # ============================
            if (flag_thread == True):
                
                print(colored(f"\n{'='*60}", "yellow"))
                print(colored(f"{n_workers} Process evaluation with ProcessPoolExecutor, Iteration {k+1}/{cem_params.cem_iters}", "yellow", attrs=['bold']))
                print(colored(f"{'='*60}\n", "yellow"))
                
                # Submit all population tasks to the persistent pool
                args_list = [(i, inputs[i]) for i in range(cem_params.pop_size)]
                future_to_index = {_process_executor.submit(_eval_with_timing_process, arg): arg[0]
                ...
                for future in as_completed(future_to_index):
                    ...
                    # Results arrive in main process serially via as_completed, no lock needed
        ...
        # Shutdown the pool: workers receive clean exit signal and run atexit handlers
        # (atexit.register(close_matlab_engines) in _worker_init closes each worker's MATLAB engine)
        if _process_executor is not None:
            _process_executor.shutdown(wait=True)
        # Close the main process MATLAB engine
...
if __name__ == "__main__":
    try:
        ...
        # Ensure pool and all MATLAB engines are closed even if main() raises
        if _process_executor is not None:
            _process_executor.shutdown(wait=True)
        close_matlab_engines()