...
# 1. P0_INIT
WAYPOINTS = [
    np.array([0.5, 4.09, -40.]),   # P0 (Partenza)
    np.array([0.5, 3.39, -35.0]),     # P1 (Tappa 1)
]
P0_INIT = WAYPOINTS[0]
PF_INIT = WAYPOINTS[-1]
MAX_JUMP = 6
...
flag_thread = True
patience = 3
CORRIDOR_RADIUS = 3.0 # for linear corridor warm start
# MAIN_DIRECTORY = "result/2_test"

MAIN_DIRECTORY = os.environ.get("EXPERIMENT_DIR", "result/multi_step_11")
...
fitness_weights = np.array([1e5, 1., 0., 100., 40.0,  0.0]) # Optimizer
...
filter_weights = np.array([0., 1., 1.0, 0.0, 0.0])
...
terrain_type = os.environ.get("TERRAIN_TYPE", "mixed_big_gaussians") #custom_gaussians | hemisphere | rock | gaussian_bumps
...
def initialize_terrain_data(p0, pf, warm_start_mode=False):
    ...
    # patches.visualize_full_cost_map_with_endpoints(p0, pf, elev=10, azim=-10)
    ...
    patch_pf = patches.get_patch_id_from_point_2D(pf[1], pf[2])
    patch_p0 = patches.get_patch_id_from_point_2D(p0[1], p0[2])
    ...
    if warm_start_mode:
        ...
        # patch_probs = get_warm_start_line_distance_only( patches, p0, pf, radius=CORRIDOR_RADIUS )
        ...
        # plot_probability_heatmap(patch_probs, patches, p0, pf)
...
def get_warm_start_base_cost(patches, sensitivity=2.0): 
...
def plot_probability_heatmap(patch_probs, patches_obj, p0, pf):
    ...
    pc.set_array(patch_probs)
    ax.add_collection(pc)
    
    # Barra laterale (Colorbar)
    # cbar = fig.colorbar(pc, ax=ax, fraction=0.046, pad=0.04)
    # cbar.set_label('Warm Start Probability', rotation=270, labelpad=20, fontsize=14)
    # cbar.ax.tick_params(labelsize=10)

    # 3. Annotazioni Testuali (ID e %) — font piccolo per non sovrapporre
    for item in ids_and_probs:
        ax.text(item['y'], item['z'], f"{item['id']}\n{item['p']:.1%}", 
                ha='center', va='center', fontsize=8, color='black', fontweight='bold')

    # =========================================================
    # 4. START & GOAL (Coordinate Y, Z)
    # =========================================================
    ax.scatter(p0[1], p0[2], c='blue', s=200, marker='o', edgecolors='white', zorder=10, label='Start')
    ax.text(p0[1], p0[2] + (height * 0.8), "START", ha='center', color='blue', fontweight='bold', fontsize=9, zorder=10)
    
    ax.scatter(pf[1], pf[2], c='gold', s=300, marker='*', edgecolors='black', zorder=10, label='Goal')
    ax.text(pf[1], pf[2] - (height * 0.8), "GOAL", ha='center', color='orange', fontweight='bold', fontsize=9, zorder=10)

    # Configurazione Assi
    ax.set_xlabel("Y (Width)", fontsize=14, labelpad=5)
    ax.set_ylabel("Z (Height)", fontsize=14, labelpad=5)
    ax.legend(loc='upper right', fontsize=10)
    ax.tick_params(axis='both', which='major', labelsize=11)
    ax.grid(True, linestyle='--', alpha=0.3)
    
    # Centra la mappa con margine proporzionale
    margin_y = width * 1.0
    margin_z = height * 1.0
    ax.set_xlim(patches_obj.y_min - margin_y, patches_obj.y_max + margin_y)
    ax.set_ylim(patches_obj.z_min - margin_z, patches_obj.z_max + margin_z)
    ax.set_aspect('equal')
