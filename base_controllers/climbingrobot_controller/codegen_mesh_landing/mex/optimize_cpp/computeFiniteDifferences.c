    ...
    const emxArray_real_T *lb, const emxArray_real_T *ub)
{
  ...
  while ((!exitg1) && (idx <= obj->nVar - 1)) {
    ...
    ubDiff = xk_data[idx];
    xk_data[idx] += deltaX;
    lbDiff = optimize_cpp_anonFcn1(obj->objfun.workspace.p0,
                                   &obj->objfun.workspace.params, xk);
    evalOK = ((!muDoubleScalarIsInf(lbDiff)) && (!muDoubleScalarIsNaN(lbDiff)));
    if (evalOK) {
    ...
    if (!evalOK) {
      if (!modifiedStep) {
        ...
        if ((!obj->hasBounds) || modifiedStep) {
          ...
          lbDiff = optimize_cpp_anonFcn1(obj->objfun.workspace.p0,
                                         &obj->objfun.workspace.params, xk);
          evalOK = ((!muDoubleScalarIsInf(lbDiff)) &&
                    (!muDoubleScalarIsNaN(lbDiff)));
          if (evalOK) {
            ...
            b_idx = 0;
            while (evalOK && (b_idx + 1 <= obj->mIneq)) {
              evalOK = ((!muDoubleScalarIsInf(r1[b_idx])) &&
                        (!muDoubleScalarIsNaN(r1[b_idx])));
              b_idx++;
            }
            if (evalOK) {
              xk_data[idx] = ubDiff;
