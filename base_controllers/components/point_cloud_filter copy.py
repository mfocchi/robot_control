...
class PointCloudFilter:
    ...
    def compute_conv_step(self, kernel, source_points=None,plot=False):
        ...
        if plot:
            ...
            self.plot_filter_response_map_2d(self.surface, self.grid_y, self.grid_z)
            self.plot_filter_response_surface_3d(self.surface, self.grid_y, self.grid_z)
    ...
    def visualize_filter_operation(self, surface, source_points, grid_y, grid_z, elev=20, azim=-35):
    ...
    def visualize_cost_map_2(self, source_points=None, elev=20, azim=-60):
        if source_points is None:
            source_points = self.points_t
        x_points = np.array([point['position'][0] for point in source_points])
        y_points = np.array([point['position'][1] for point in source_points])
        ...
        # Figure 1: Point cloud colored by cost (3D)
        ...
        # Figure 2: Top-down view of cost map
    ...
    def plot_filter_response_map_2d(self, surface, grid_y, grid_z, alpha=0.85, figsize=(10, 8)):
        '''
        Plot only the 2D filter response map (Subplot 1 of visualize_filter_operation).
        surface: 2D array of filter response values
        grid_y: y grid values
        grid_z: z grid values
        alpha: transparency
        ...
        cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label('Response Intensity', fontsize=18)
        cbar.ax.tick_params(labelsize=14)

        ax.set_xlabel('Y (m)', fontsize=18, labelpad=12)
        ax.set_ylabel('Z (m)', fontsize=18, labelpad=12)
        ax.set_title('Filter Response Map', fontsize=20)
    ...
    def plot_filter_response_surface_3d(self, surface, grid_y, grid_z, alpha=0.85, figsize=(10, 8), elev=20, azim=-35):
        '''
        Plot only the 3D filter response surface (Subplot 3 of visualize_filter_operation).
        surface: 2D array of filter response values
        grid_y: y grid values
        grid_z: z grid values
        alpha: transparency
        ...
        ax.set_xlabel('Y (m)', fontsize=18, labelpad=12)
        ax.set_ylabel('Z (m)', fontsize=18, labelpad=12)
        ax.set_zlabel('Filter Response', fontsize=18, labelpad=12)
        ax.set_title('3D Filter Response Surface', fontsize=20)
...
def main():
    ...
    terrain  = TerrainManager(grid_size=100,wall_depth =1,max_ridge_depth=0.5, seed="default", Lz=-10, Ly=10, generate_terrain=True, terrain_type="rock")

    pc = terrain.point_cloud
    # Point cloud filter test
    pc_filter = PointCloudFilter(pc, h_min=.10, h_max=4.0)
    ...
    # pc_filter.filter_height_profile(x0=0.0, scale=.10,side_application="both", profile="logln")
    # pc_filter.visualize_cost_map_2(elev=20, azim=-35)
