...
if friction_coeff == '0.1':
    WLmax = 9.5
elif friction_coeff == '0.4' or friction_coeff == '0.6':
    WLmax = 18
else:
...
list_file = [x for x in os.listdir('data2d/'+friction_coeff) if (x.endswith('.csv') )]
...
for file in list_file:
    print("reading...",file)
    tmp_df = pd.read_csv(os.path.join('data2d/'+friction_coeff+'/',file),header=1, names=['time', 'wheel_l','wheel_r','roll', 'pitch', 'yaw', 'beta_l','beta_r','alpha'])
...
# Apply filter
idx_filter = (v > min_long_speed)  & (np.abs(wheel_l) <= WLmax) &  (np.abs(wheel_r) <= WLmax)
