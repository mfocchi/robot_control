...
Implement a Joint $PD$ control law with proportional $K_p$ = 300 $Nm/rad$ and derivative $K_d$ = 20 $Nms/rad$ gains for all joints. Use the step reference generated in (1.2). Plot the reference and the actual joint position for all joints and see that a tracking error is present both during the transient and at steady state. Explain why there is almost no tracking error in the $5^{th}$ joint. % because Kp for the inertia is very stiff
