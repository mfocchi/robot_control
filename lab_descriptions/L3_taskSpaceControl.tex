...
First define the rotation matrix that represents the desired orientation ${}_wR_d = [\hat{x}^d, \hat{y}^d, \hat{z}^d]$ with  ($\hat{x}_d$ axis along $\hat{x}_w$, $\hat{y}_d$ axis  along $-\hat{y}_w$, $\hat{z}_d$ axis along $-\hat{z}_w$).
