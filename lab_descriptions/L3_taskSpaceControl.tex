...
First define the rotation matrix that represents the desired orientation, ${}_wR_d = [\hat{x}^d, \hat{y}^d, \hat{z}^d]$ with  ($\hat{x}_d$ axis along $\hat{x}_w$, $\hat{y}_d$ axis  along $-\hat{y}_w$, $\hat{z}_d$ axis along $-\hat{z}_w$). This is equivalent to rotate the world frame 180 degrees with respect to its $X$ axis. % for some reason the result is opposite for rotations smaller that 180 degs TODO
...
%With angle-axis we still have a singularity at zero but we can deal with that with the atan2(.). 
